# Fish Audio Live — OpenClaw Plugin Design
...
1. Ship an OpenClaw plugin that registers a `SpeechProvider` named `fish-audio-live`, using Fish Audio's WebSocket Live endpoint as its primary transport.
...
| 9 | Plugin id | `fish-audio-live` | Avoids collision with Conan Scott's `fish-audio` so both can be installed simultaneously. |
| 10 | npm package | `@averatec0773/openclaw-fish-audio-live` | Matches repo name. |
